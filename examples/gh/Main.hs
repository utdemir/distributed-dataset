...
app =
  ...
  ghArchive (fromGregorian 2018 1 1, fromGregorian 2018 12 31) & -- Extract every commit
    dConcatMap
      ( static
        ( \e ->
  ...
  &
    dGroupedAggr 50 (static fst) aggrCount & -- Fetch the top 20 to driver as a list
    dAggr (aggrTopK (static Dict) 20 (static snd)) >>= -- Print them
    mapM_ (liftIO . print)
