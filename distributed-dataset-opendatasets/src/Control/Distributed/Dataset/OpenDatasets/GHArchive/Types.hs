...
instance FromJSON GHEvent where
  parseJSON val = withObject "GHEvent" (\obj ->
    GHEvent
      <$> obj .: "actor"
      <*> obj .: "repo"
      <*> parseJSON val
...
data GHEventType = GHPushEvent GHPushEventPayload
                 | GHOtherEvent Text
  deriving (Eq, Show, Generic, Serialise)
...
instance FromJSON GHPushEventPayload where
  parseJSON = withObject "GHPushEventPayload" $ \obj->
    GHPushEventPayload
      <$> obj .: "commits"
...
instance FromJSON GHCommit where
  parseJSON = withObject "GHCommit" $ \obj->
    GHCommit
      <$> obj .: "author"
      <*> obj .: "message"
      <*> obj .: "sha"
...
instance FromJSON GHCommitAuthor where
  parseJSON = withObject "GHCommitAuthor" $ \obj->
    GHCommitAuthor
      <$> obj .: "email"
      <*> obj .: "name"
...
instance FromJSON GHActor where
  parseJSON = withObject "GHActor" $ \obj->
    GHActor
      <$> obj .: "id"
      <*> obj .: "login"
...
instance FromJSON GHRepo where
  parseJSON = withObject "GHRepo" $ \obj->
    GHRepo
      <$> obj .: "id"
      <*> obj .: "name"
